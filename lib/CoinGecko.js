...
class CoinGecko {
  ...
  _buildRequestOptions(path, params) {
    ...
    if (Utils.isObject(params)) params = Utils.createQueryStringFromObject(params);
  ...
  _request(path, params) {
    ...
    try {
      ...
      const url = `https://${options.host}:${options.port}${options.path}`

      if ('fetch' in window) {
        return fetch(url, { method: options.method, timeout: options.timeout });
      } else {
